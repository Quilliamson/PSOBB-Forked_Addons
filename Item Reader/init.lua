...
if optionsLoaded then
	...
	options.showMaxStats              = lib_helpers.NotNilOrDefault(options.showMaxStats, false)
...
local function SaveOptions(options)
    ...
    if file ~= nil then
		...
		io.write(string.format("    showMaxStats = %s,\n", tostring(options.showMaxStats)))
...
local function ProcessFrame(item)
	...
	local statColor
	if options.showMaxStats then
		if item.armor.dfp == item.armor.dfpMax and item.armor.evp == item.armor.evpMax and (item.armor.dfpMax > 0 or item.armor.evpMax > 0) then
			result = result .. lib_helpers.TextC(false, 0xFFFFD700, "%s ", TrimString(item.name, options.itemNameLength))
		else
			result = result .. lib_helpers.TextC(false, nameColor, "%s ", TrimString(item.name, options.itemNameLength))
		end
	else
    ...
    result = result .. lib_helpers.TextC(false, lib_items_cfg.white, "[D ")
	
    if options.showMaxStats then
		if item.armor.dfp == 0 then
			statColor = lib_items_cfg.grey
		elseif item.armor.dfp == item.armor.dfpMax then
			statColor = 0xFFFFD700
		else
	...
	if options.showMaxStats then
		if item.armor.dfpMax == 0 then
			statColor = lib_items_cfg.grey
		elseif item.armor.dfpMax == item.armor.dfp then
			statColor = 0xFFFFD700
		else
			statColor = lib_items_cfg.armorStats
		end
    ...
    result = result .. lib_helpers.TextC(false, lib_items_cfg.white, " E ")
	if options.showMaxStats then
		if item.armor.evp == 0 then
			statColor = lib_items_cfg.grey
		elseif item.armor.evp == item.armor.evpMax then
			statColor = 0xFFFFD700
		else
			statColor = lib_items_cfg.armorStats
		end
	...
	if options.showMaxStats then
		if item.armor.evpMax == 0 then
			statColor = lib_items_cfg.grey
		elseif item.armor.evpMax == item.armor.evp then
			statColor = 0xFFFFD700
		else
			statColor = lib_items_cfg.armorStats
		end
...
local function PresentInventory(save, index)
    ...
    lib_helpers.TextC(false, lib_items_cfg.itemIndex, "$%i", cache_inventory.meseta)
    lib_helpers.TextC(true, lib_items_cfg.itemIndex, "Inv: %i/30", itemCount)
