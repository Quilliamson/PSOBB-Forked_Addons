...
local function LoadConfiguration()
	...
	_Configuration.sessionInfoMeseta = (_Configuration.sessionInfoMeseta ~= nil) and _Configuration.sessionInfoMeseta
	_Configuration.sessionInfoMPH = (_Configuration.sessionInfoMPH ~= nil) and _Configuration.sessionInfoMPH
	_Configuration.sessionInfoPHiD = (_Configuration.sessionInfoMPHiD ~= nil) and _Configuration.sessionInfoMPHiD
	_Configuration.sessionInfoExp = (_Configuration.sessionInfoExp ~= nil) and _Configuration.sessionInfoExp
	_Configuration.sessionInfoExpPH = (_Configuration.sessionInfoExpPH ~= nil) and _Configuration.sessionInfoExpPH
	_Configuration.sessionInfoExpPHiD = (_Configuration.sessionInfoExpPHiD ~= nil) and _Configuration.sessionInfoExpPHiD
	_Configuration.sessionInfoTime = (_Configuration.sessionInfoTime ~= nil) and _Configuration.sessionInfoTime
	_Configuration.sessionInfoDTime = (_Configuration.sessionInfoDTime ~= nil) and _Configuration.sessionInfoDTime
	_Configuration.sessionInfoNoTitleBar = (_Configuration.sessionInfoNoTitleBar ~= nil) and _Configuration.sessionInfoNoTitleBar
	_Configuration.sessionInfoMoveLocked = (_Configuration.sessionInfoMoveLocked ~= nil) and _Configuration.sessionInfoMoveLocked
	_Configuration.sessionInfoResizeLocked = (_Configuration.sessionInfoResizeLocked ~= nil) and _Configuration.sessionInfoResizeLocked
    ...
    _Configuration.serialize = function(configurationPath)
        ...
        if file ~= nil then
			...
			io.write("\n")
			io.write(string.format("    sessionInfoMeseta = %s,\n", _Configuration.sessionInfoMeseta))
			io.write(string.format("    sessionInfoMPH = %s,\n", _Configuration.sessionInfoMPH))
			io.write(string.format("    sessionInfoMPHiD = %s,\n", _Configuration.sessionInfoMPHiD))
			io.write(string.format("    sessionInfoExp = %s,\n", _Configuration.sessionInfoExp))
			io.write(string.format("    sessionInfoExpPH = %s,\n", _Configuration.sessionInfoExpPH))
			io.write(string.format("    sessionInfoExpPHiD = %s,\n", _Configuration.sessionInfoExpPHiD))
			io.write(string.format("    sessionInfoTime = %s,\n", _Configuration.sessionInfoTime))
			io.write(string.format("    sessionInfoDTime = %s,\n", _Configuration.sessionInfoDTime))
			io.write(string.format("    sessionInfoNoTitleBar = %s,\n", _Configuration.sessionInfoNoTitleBar))
            io.write(string.format("    sessionInfoMoveLocked = %s,\n", _Configuration.sessionInfoMoveLocked))
			io.write(string.format("    sessionInfoResizeLocked = %s,\n", tostring(_Configuration.sessionInfoResizeLocked)))
            io.write("\n")
...
local function ConfigurationWindow(configuration)
    ...
    local _showSessionCounterSettings = function()
        ...
        if imgui.TreeNodeEx("Session Kill Counters") then
			...
			if imgui.Checkbox("No Info resize", _configuration.sessionInfoResizeLocked) then
                _configuration.sessionInfoResizeLocked = not _configuration.sessionInfoResizeLocked
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
            if imgui.Checkbox("No Info move", _configuration.sessionInfoMoveLocked) then
                _configuration.sessionInfoMoveLocked = not _configuration.sessionInfoMoveLocked
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
            if imgui.Checkbox("No Info title bar", _configuration.sessionInfoNoTitleBar) then
                _configuration.sessionInfoNoTitleBar = not _configuration.sessionInfoNoTitleBar
                _hasChanged = true
            end
    ...
    local _showAdvancedSettings = function()
        ...
        if imgui.TreeNodeEx("Advanced") then
            ...
            if imgui.Checkbox("Meseta", _configuration.sessionInfoMeseta) then
                _configuration.sessionInfoMeseta = not _configuration.sessionInfoMeseta
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
			if imgui.Checkbox("Meseta per hour", _configuration.sessionInfoMPH) then
                _configuration.sessionInfoMPH = not _configuration.sessionInfoMPH
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
			if imgui.Checkbox("Meseta per hour in dungeon", _configuration.sessionInfoMPHiD) then
                _configuration.sessionInfoMPHiD = not _configuration.sessionInfoMPHiD
                _hasChanged = true
            end
			
			if imgui.Checkbox("Experience", _configuration.sessionInfoExp) then
                _configuration.sessionInfoExp = not _configuration.sessionInfoExp
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
			if imgui.Checkbox("Experience per hour", _configuration.sessionInfoExpPH) then
                _configuration.sessionInfoExpPH = not _configuration.sessionInfoExpPH
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
			if imgui.Checkbox("Experience per hour in dungeon", _configuration.sessionInfoExpPHiD) then
                _configuration.sessionInfoExpPHiD = not _configuration.sessionInfoExpPHiD
                _hasChanged = true
            end
			
			if imgui.Checkbox("Time", _configuration.sessionInfoTime) then
                _configuration.sessionInfoTime = not _configuration.sessionInfoTime
                _hasChanged = true
            end
			
			imgui.SameLine(0, 50)
			if imgui.Checkbox("Dungeon Time", _configuration.sessionInfoDTime) then
                _configuration.sessionInfoDTime = not _configuration.sessionInfoDTime
                _hasChanged = true
            end
...
local function init()
	...
	_SessionInfoWindow.Meseta = _Configuration.sessionInfoMeseta
	_SessionInfoWindow.MPH = _Configuration.sessionInfoMPH
	_SessionInfoWindow.MPHiD = _Configuration.sessionInfoMPHiD
	_SessionInfoWindow.Exp = _Configuration.sessionInfoExp
	_SessionInfoWindow.ExpPH = _Configuration.sessionInfoExpPH
	_SessionInfoWindow.ExpPHiD = _Configuration.sessionInfoExpPHiD
	_SessionInfoWindow.Time = _Configuration.sessionInfoTime
	_SessionInfoWindow.DTime = _Configuration.sessionInfoDTime
	_SessionInfoWindow.titleBar = (_Configuration.sessionInfoNoTitleBar and 'NoTitleBar' or '')
	_SessionInfoWindow.move = (_Configuration.sessionInfoMoveLocked and 'NoMove' or '')
	_SessionInfoWindow.resize = (_Configuration.sessionInfoResizeLocked and 'NoResize' or '')
